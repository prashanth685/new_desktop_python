...
class SubToolBar(QWidget):
    ...
    def toggle_blink(self):
        ...
        if self.recording_indicator:
            ...
            self.recording_indicator.setVisible(self.blink_state and self.is_saving)
            logging.debug(f"SubToolBar: Blink state: {self.blink_state}, indicator visible: {self.recording_indicator.isVisible()}")
    ...
    def update_subtoolbar(self):
        ...
        """)
        self.recording_indicator.setVisible(False)  # Initially hidden
        self.toolbar.addWidget(self.recording_indicator)
        # self.toolbar.addSeparator()

        # Initialize QLineEdit for filename
        self.filename_edit = QLineEdit()
        self.filename_edit.setStyleSheet("""
            QLineEdit {
                background-color: #ffffff;
        ...
        # Determine if any Time View instance is saving
        self.is_saving = False
        selected_model = self.parent.tree_view.get_selected_model() if hasattr(self.parent, 'tree_view') else None
        if is_time_view and selected_model:
            time_view_keys = [k for k in self.parent.feature_instances.keys() if k[0] == "Time View" and k[1] == selected_model]
            if time_view_keys:
                # Check the most recent Time View instance
                key = max(time_view_keys, key=lambda k: k[3])
                feature_instance = self.parent.feature_instances.get(key)
                self.is_saving = getattr(feature_instance, 'is_saving', False)

        # Start or stop blinking based on is_saving
        if self.is_saving and not self.blink_timer.isActive():
            self.blink_timer.timeout.connect(self.toggle_blink)
            self.blink_timer.start()
            logging.debug("SubToolBar: Started blinking timer")
        elif not self.is_saving and self.blink_timer.isActive():
            self.blink_timer.stop()
            self.recording_indicator.setVisible(False)
            self.blink_state = False
            logging.debug("SubToolBar: Stopped blinking timer")

        # Enable save/stop actions only for Time View
        add_action("▶", "#ffffff", self.parent.start_saving, "Start Saving Data (Time View)", is_time_view and not self.is_saving, "#43a047")
        add_action("⏸", "#ffffff", self.parent.stop_saving, "Stop Saving Data (Time View)", is_time_view and self.is_saving, "#90a4ae")
        self.toolbar.addSeparator()
    ...
    def refresh_filename(self):
        ...
        try:
            ...
            # Check if in Time View and get the most recent Time View instance
            if self.parent.current_feature == "Time View":
                selected_model = self.parent.tree_view.get_selected_model() if hasattr(self.parent, 'tree_view') else None
                if selected_model:
                    time_view_keys = [k for k in self.parent.feature_instances.keys() if k[0] == "Time View" and k[1] == selected_model]
                    if time_view_keys:
                        # Get the most recent Time View instance
                        key = max(time_view_keys, key=lambda k: k[3])
                        feature_instance = self.parent.feature_instances.get(key)
                        current_filename = getattr(feature_instance, 'current_filename', None)
                        filename_counter = getattr(feature_instance, 'filename_counter', 1)
                        is_saving = getattr(feature_instance, 'is_saving', False)
                        model_name = selected_model

            # Query database for existing filenames
            filenames = self.parent.db.get_distinct_filenames(self.parent.current_project, model_name) if self.parent.current_project and model_name else []
            logging.debug(f"SubToolBar: Retrieved {len(filenames)} filenames from database: {filenames}")
