...
class OrbitFeature:
    def __init__(self, parent, db, project_name, channel=None, model_name=None, console=None):
        ...
        self.selected_pairs = [(0, 1)]  # Default: Ch1 vs Ch2
        self.cross_pair = None
        self.sample_rate = 4096
        self.samples_per_channel = 4096
        self.frequency = 10  # Hz, from MQTTPublisher
        self.amplitude = (46537 - 16390) / 2  # From MQTTPublisher
        self.offset = (46537 + 16390) / 2     # From MQTTPublisher
    ...
    def initUI(self):
        ...
        # Layout for plots (orbit and time-domain)
        self.plot_layout = QHBoxLayout()
        main_layout.addLayout(self.plot_layout)

        # Initialize with default pair (Ch1 vs Ch2)
        self.recreate_plots()
    ...
    def on_combo_changed(self, index):
        ...
        pair_map = {
            ...
            4: [(1, 2)],                    # Ch2 vs Ch3
            5: [(1, 3)],                    # Ch2 vs Ch4
            6: [(0, 1), (2, 3)]             # Ch1 vs Ch2, Ch3 vs Ch4
        }
    ...
    def recreate_plots(self):
        ...
        for ch in sorted(channels_to_plot):
            ...
            time_plot_widget.setFixedSize(400, 200)
    ...
    def update_plots_with_sine_data(self):
        ...
        self.current_time += 1.0  # Increment time as in MQTTPublisher
        phases = [0, np.pi/2, 0, np.pi/2]  # Phases for Ch1, Ch2, Ch3, Ch4 to create circular orbits
        for i in range(4):
    ...
    def on_data_received(self, tag_name, model_name, values, sample_rate):
        ...
        if not values or len(values) < 4:
            ...
            self.update_plots_with_sine_data()  # Fallback to sine waves
            return

        # Store channel data (ignore tacho data)
        ...
        for i, (ch_x, ch_y) in enumerate(self.selected_pairs):
            ...
            # Calculate circularity for (1,2) and (3,4)
            if (ch_x, ch_y) in [(0, 1), (2, 3)]:
                radius = np.sqrt((x_data - self.offset)**2 + (y_data - self.offset)**2)
                mean_radius = np.mean(radius)
                std_radius = np.std(radius)
