...
class TabularViewFeature:
    def __init__(self, parent, db, project_name, channel=None, model_name=None, console=None):
        self.parent = parent
        self.db = db  # Store db reference for refreshing properties
        self.project_name = project_name
        self.channel = channel
        self.model_name = model_name
    ...
    def process_calibrated_data(self, data, channel_idx):
        ...
        props = self.channel_properties.get(channel_name, {"Unit": "mil", "CorrectionValue": 1.0, "Gain": 1.0, "Sensitivity": 1.0, "ConvertedSensitivity": 1.0})
        # Use ConvertedSensitivity for calibration (based on edits)
        channel_data = np.array(data[channel_idx], dtype=float) * (3.3 / 65535.0) * (props["CorrectionValue"] * props["Gain"]) / props["ConvertedSensitivity"]
    ...
    def refresh_channel_properties(self):
        """Reload channel properties from DB and recalculate if edited."""
        try:
            ...
            if model:
                ...
                for channel in model["channels"]:
                    ...
                    self.channel_properties[channel_name] = {
                        ...
                        "ConvertedSensitivity": channel.get("ConvertedSensitivity", 1.0),  # Use calculated field
                        # Add other fields as needed
                    }
                if self.console:
                    self.console.append_to_console("Refreshed channel properties from DB")
                self.update_table_defaults()  # Refresh table
                self.update_plots()  # Recalculate plots
        except Exception as ex:
            logging.error(f"Error refreshing channel properties: {str(ex)}")
