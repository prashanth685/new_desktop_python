...
#         # Top layout with settings and channel selector
...
#             self.initialize_plots()  # Ensure plots are initialized even if table fails
#             return
#         try:
...
#             self.initialize_plots()
#             return
#         ch = 0  # Always use Channel 1 for plots
#         if ch >= self.num_channels:
#             self.log_and_set_status("Channel 1 not available, skipping plot update")
#             return
#         trim_samples = 47
#         low_pass_trim = 0
#         high_pass_trim = 110
...
#                         "Unit": channel.get("Unit", "mil").lower(),
#                         "CorrectionValue": float(channel.get("CorrectionValue", "1.0")),
#                         "Gain": float(channel.get("Gain", "1.0")),
#                         "Sensitivity": float(channel.get("Sensitivity", "1.0")) if channel.get("Sensitivity") else 1.0
#                     }
#                 if self.console:
#                     self.console.append_to_console("Refreshed channel properties from DB")
#                 self.update_table_defaults()
#                 self.update_plots()
#         except Exception as ex:
...
class TabularViewFeature:
    ...
    def on_channel_selected(self, index):
        if self.table and self.table_initialized:
            ...
            self.update_plots()
            if self.console:
                self.console.append_to_console(f"Switched to channel: {self.channel_names[self.selected_channel]}")

    def on_data_received(self, tag_name, model_name, values, sample_rate, headers=None):
        if not values or len(values) < 1:
            self.log_and_set_status(f"Insufficient data received: {len(values)} channels")
            return
        try:
            # Update channel names from headers[2] (main_channels)
            if headers and len(headers) > 2:
                main_channels = headers[2]
                if isinstance(main_channels, str):
                    self.channel_names = [ch.strip() for ch in main_channels.split(',')] if main_channels else ["Channel 1"]
                else:
                    self.channel_names = main_channels if main_channels else ["Channel 1"]
            ...
            # Pad or truncate values to match num_channels
            ...
            self.data = values
            if self.console:
                self.console.append_to_console(f"Received data for {len(values)} channels, updated channels: {self.channel_names}")

            # Process all channels and build table data
            channel_data_list = []
            for ch in range(self.num_channels):
                ...
                self.high_pass_data[ch] = signal.lfilter(high_pass_coeffs, 1.0, self.raw_data[ch])
                self.band_pass_data[ch] = signal.lfilter(band_pass_coeffs, 1.0, self.raw_data[ch])
                self.average_frequency[ch] = 0.0  # Adjust if frequency data is available
            ...
            self.update_plots()
            if self.console:
                self.console.append_to_console(f"Updated table with data for all {self.num_channels} channels")
        except Exception as ex:
            self.log_and_set_status(f"Error processing data: {str(ex)}")
